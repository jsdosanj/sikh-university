...
| `.glass-hall` | Opt-in only, `ambient="hall"` pages — see the hall exception below |
...
- **Hall exception:** pages with `ambient="hall"` (the arrival/marketing surfaces) may run
  up to **8** live `backdrop-filter` elements, and only via the explicit `.glass-hall` class
  — never by scoping `.glass-lite` up to full glass, which would silently blur an entire
  card grid. `.glass-hall` demotes to the lite form (no backdrop-filter) at ≤1024px, wider
  than `.glass-card`'s ≤640px, since the effect is desktop-first grandeur, not a mobile
  affordance. `study`, `sanctum`, and every reading surface stay at 6 — this is a deliberate,
  scoped exception for first-impression pages, not a budget increase.
...
|---|---|---|
| `hall` | index, about, programs, login, 404 | grand navy, two slow-drifting radial glows + lattice |
| `study` | catalog, course, dashboard, search, professors, cert | warm paper, manuscript grain, static saffron top glow |
...
- [ ] ≤6 live backdrop-filter elements at any scroll position (≤8 on `ambient="hall"` pages,
      via `.glass-hall` only — audit snippet in docs/SEO QA)
